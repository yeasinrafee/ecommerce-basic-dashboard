...
const OtpInput = React.forwardRef<HTMLDivElement, OtpInputProps>((props, ref) => {
  ...
  const computeInitial = () => {
    if (expiry) {
      const end = typeof expiry === "number" ? expiry : new Date(expiry).getTime();
  ...
  useEffect(() => {
    let mounted = true;
    const end = expiry ? (typeof expiry === "number" ? expiry : new Date(expiry).getTime()) : Date.now() + timerDuration * 1000;
  ...
  return (
    <div ref={ref} className={clsx("flex flex-col gap-4", className)}>
      <div className="flex flex-col gap-2 relative z-10">
        ...
        <div className={clsx("flex w-full gap-3 justify-center py-4", inputWrapperClassName)}>
          {Array.from({ length }).map((_, i) => (
            <input
              ...
              className={clsx(
                ...
                value[i] ? "border-indigo-400 text-indigo-900 bg-indigo-50/30" : "border-slate-200 text-slate-900",
                slotClassName
              )}
            />
          ))}
        ...
        <div className="flex flex-col items-center mt-4 space-y-3">
          <div className="text-center">
            <span className={clsx(
              ...
              timeLeft <= 30 ? "text-rose-600 bg-rose-50 border-rose-100 animate-pulse" : "text-slate-700 bg-slate-50 border-slate-100"
            )}>
              {formatTime(timeLeft)}
            </span>
          </div>
        ...
        {error ? (
          <p className={clsx("text-sm text-center font-medium mt-2 text-rose-500", errorClassName)}>{error}</p>
        ) : helperText ? (
          <p className={clsx("text-sm text-center mt-2 text-slate-500", helperClassName)}>{helperText}</p>
        ) : null}
      </div>
    </div>
  );
