...
import { useAllSocialMedia, useCreateSocialMedia, useUpdateSocialMedia, useDeleteSocialMedia, SocialMedia } from "@/hooks/web.api"
...
const SocialMediaLinkForm = () => {
  ...
  useEffect(() => {
    if (existingLinks && existingLinks.length > 0) {
      setRows(existingLinks.map(link => ({ id: link.id, name: link.name, link: link.link })))
    }
  }, [existingLinks])
  ...
  const handleInputChange = (index: number, field: keyof SocialMediaRow, value: string) => {
    const newRows = [...rows]
  ...
  const allMandatoryFieldsFilled = rows.every(row => row.name.trim() !== "" && row.link.trim() !== "")

  const hasChanges = useMemo(() => {
    if (!existingLinks) return rows.some(row => row.name !== "" || row.link !== "")
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8 bg-background rounded-xl shadow-sm border border-slate-200">
      <div className="flex justify-between items-center">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500">Manage your company's social media presence.</p>
        </div>
      </div>

      <div className="space-y-6">
        {rows.map((row, index) => (
          <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-slate-50/50 p-4 rounded-lg border border-slate-100">
            <div className="md:col-span-5">
              <CustomInput
                ...
                value={row.name}
                onValueChange={(v) => handleInputChange(index, "name", v as string)}
              />
            </div>
            <div className="md:col-span-6">
              <CustomInput
                ...
                value={row.link}
                onValueChange={(v) => handleInputChange(index, "link", v as string)}
              />
            </div>
      ...
      <div className="flex justify-center pt-6 border-t border-slate-100">
        ...
        >
          {existingLinks && existingLinks.length > 0 ? "Update Links" : "Save Links"}
        </CustomButton>
      </div>
    </div>
  )
